...
class BreakTimesController extends Controller
{
     ...
     * @return BreakTime[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Http\JsonResponse
     */
    public function api_get_all() {
        $this->module = get_user_module_security($this->module_key);
        $break_times = self::get_all();
    ...
    public function api_get_list()
    {
        ...
        if (!empty($this->module->read)) {
            $user = get_loged_user();
            return BreakTime::select('id', 'name as label')
    ...
    public function api_store(Request $request) {
        ...
        if(!empty($this->module->create)) {
            $break_time = self::create($request->all());

            if(empty($break_time['errors'])) {
                return $break_time;
            } else {
                return $break_time;
            }
    ...
    public function api_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $break_time = self::update($request->all(), $id);
    ...
    private function get_all() {
        ...
        if(!empty($this->module->read)) {
            $user = get_loged_user();
            return BreakTime::where('company_id', $user->company_id)->get();
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    }
    ...
    private function get($id) {
        ...
        if (!empty($this->module->read)) {
            $user = get_loged_user();
            $break_time = BreakTime::findOrFail($id);
            if($user->company_id == $break_time->company_id) {
                return $break_time;
            } else {
                return null;
            }
        ...
        if(!empty($this->module->read)) {
            return BreakTime::findOrFail($id);
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    }
