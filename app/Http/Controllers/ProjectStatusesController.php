...
class ProjectStatusesController extends Controller
{
     ...
     * @return ProjectStatus[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Http\JsonResponse
     */
    public function api_get_all() {
        $this->module = get_user_module_security($this->module_key);
        $project_statuses = self::get_all();
    ...
    public function api_get_list()
    {
        ...
        if (!empty($this->module->list)) {
            $user = get_loged_user();
            return ProjectStatus::select('id', 'name as label')
    ...
    public function api_store(Request $request) {
        ...
        if(!empty($this->module->create)) {
            $project_status = self::create($request->all());

            if(empty($project_status['errors'])) {
                return $project_status;
            } else {
                return $project_status;
            }
    ...
    public function api_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $project_status = self::update($request->all(), $id);
    ...
    private function get_all() {
        ...
        if(!empty($this->module->read)) {
            $user = get_loged_user();
            return ProjectStatus::where('company_id', $user->company_id)->get();
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    }
    ...
    private function get($id) {
        ...
        if (!empty($this->module->read)) {
            ...
            if($project_status->company_id != $user->company_id) {
                return null;
            } else {
    ...
    private function update (Array $data, int $id) {
        ...
        if ($validator->fails()) {
            ...
            $project_status = ProjectStatus::findOrFail($id);
            $user = get_loged_user();
            if($user->company_id == $project_status->company_id) {
                $project_status->update($data);
            }
