...
class AddressBooksController extends Controller
{
     ...
     * @return AddressBook[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Http\JsonResponse
     */
    public function api_get_all() {
        $this->module = get_user_module_security($this->module_key);
        $address_books = self::get_all();
        if (!empty($address_books)) {
            return $address_books->load('department');
        } else {
    ...
    public function api_get_list()
    {
        ...
        if (!empty($this->module->read)) {
            $user = get_loged_user();
            return AddressBook::select('id', 'name as label')
                ->get();
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    ...
    public function api_get(int $id) {
        ...
        $address_book = self::get($id);
        if (!empty($address_book)) {
            return $address_book->load('department');
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    }
    ...
    public function api_store(Request $request) {
        ...
        if(!empty($this->module->create)) {
            $address_book = self::create($request->all());

            if(empty($address_book['errors'])) {
                return $address_book->load('department');
            } else {
                return $address_book;
            }
    ...
    public function api_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $address_book = self::update($request->all(), $id);
    ...
    private function get_all() {
        ...
        if(!empty($this->module->read)) {
            $user = get_loged_user();
            return AddressBook::where('company_id', $user->company_id)->get();
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    }
    ...
    private function get($id) {
        ...
        if (!empty($this->module->read)) {
            ...
            if($user->company_id == $address_book->company_id) {
                return $address_book;
            } else {
                return null;
            }
