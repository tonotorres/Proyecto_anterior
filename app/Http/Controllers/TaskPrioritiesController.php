...
class TaskPrioritiesController extends Controller
{
     ...
     * @return TaskPriority[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Http\JsonResponse
     */
    public function api_get_all() {
        $this->module = get_user_module_security($this->module_key);
        $task_priorities = self::get_all();
        if (!empty($task_priorities)) {
    ...
    public function api_get_list()
    {
        ...
        if (!empty($this->module->list)) {
            $user = get_loged_user();
            return TaskPriority::select('id', 'name as label')
    ...
    public function api_store(Request $request) {
        ...
        if(!empty($this->module->create)) {
            $task_priority = self::create($request->all());

            if(empty($task_priority['errors'])) {
                return $task_priority;
            } else {
                return $task_priority;
            }
    ...
    public function api_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $task_priority = self::update($request->all(), $id);
    ...
    private function get_all() {
        ...
        if(!empty($this->module->read)) {
            $user = get_loged_user();
            return TaskPriority::where('company_id', $user->company_id)->get();
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    }
    ...
    private function get($id) {
        ...
        if (!empty($this->module->read)) {
            ...
            if($task_priority->company_id != $user->company_id) {
                return null;
            } else {
    ...
    private function update (Array $data, int $id) {
        ...
        if ($validator->fails()) {
            ...
            $task_priority = TaskPriority::findOrFail($id);
            $user = get_loged_user();
            if($user->company_id == $task_priority->company_id) {
