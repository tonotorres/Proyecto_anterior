...
class CompanyConfigsController extends Controller
{
     ...
     * @return CompanyConfig[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Http\JsonResponse
     */
    public function api_get_all() {
        $this->module = get_user_module_security($this->module_key);
        $company_configs = self::get_all();
        if (!empty($company_configs)) {
    ...
    public function api_store(Request $request) {
        ...
        if(!empty($this->module->create)) {
            $company_config = self::create($request->all());

            if(empty($company_config['errors'])) {
                return $company_config;
            } else {
                return $company_config;
            }
    ...
    public function api_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $company_config = self::update($request->all(), $id);
    ...
    private function get_all() {
        ...
        if(!empty($this->module->read)) {
            $user = get_loged_user();
            return CompanyConfig::where('company_id', $user->company_id)->get();
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    }
    ...
    private function get($id) {
        ...
        if (!empty($this->module->read)) {
            $user = get_loged_user();
            $company_config = CompanyConfig::findOrFail($id);
            if($user->company_id == $company_config->company_id) {
                return $company_config;
            } else {
                return null;
            }
    ...
    private function create (Array $data) {
        ...
        if ($validator->fails()) {
            ...
            $company_config = CompanyConfig::create($data);

            if(!empty($data['call_types_id'])) {
                $company_config->call_types()->sync($data['call_types_id']);
            }

            if(!empty($data['departments_id'])) {
                $company_config->departments()->sync($data['departments_id']);
            }
    ...
    private function update (Array $data, int $id) {
        ...
        if ($validator->fails()) {
            ...
            if($user->company_id == $company_config->company_id) {
                $company_config->update($data);

                if(!empty($data['call_types_id'])) {
                    $company_config->call_types()->sync($data['call_types_id']);
                } else {
                    $company_config->call_types()->detach();
                }
    
                if(!empty($data['departments_id'])) {
                    $company_config->departments()->sync($data['departments_id']);
                } else {
                    $company_config->departments()->detach();
                }
            }
