...
class TaskTypesController extends Controller
{
     ...
     * @return TaskType[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Http\JsonResponse
     */
    public function api_get_all() {
        $this->module = get_user_module_security($this->module_key);
        $task_types = self::get_all();
        if (!empty($task_types)) {
            return $task_types;
        } else {
    ...
    public function api_get_list()
    {
        ...
        if (!empty($this->module->list)) {
            $user = get_loged_user();
            return TaskType::select('id', 'name as label')
    ...
    public function api_store(Request $request) {
        ...
        if(!empty($this->module->create)) {
            $task_type = self::create($request->all());

            if(empty($task_type['errors'])) {
                return $task_type;
            } else {
                return $task_type;
            }
    ...
    public function api_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $task_type = self::update($request->all(), $id);
    ...
    private function get_all() {
        ...
        if(!empty($this->module->read)) {
            $user = get_loged_user();
            return TaskType::where('company_id', $user->company_id)->get();
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    }
    ...
    private function get($id) {
        ...
        if (!empty($this->module->read)) {
            ...
            if($task_type->company_id != $user->company_id) {
                return null;
            } else {
    ...
    private function update (Array $data, int $id) {
        ...
        if ($validator->fails()) {
            ...
            $task_type = TaskType::findOrFail($id);
            $user = get_loged_user();
            if($user->company_id == $task_type->company_id) {
