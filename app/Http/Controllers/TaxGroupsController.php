...
class TaxGroupsController extends Controller
{
     ...
     * @return TaxGroup[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Http\JsonResponse
     */
    public function api_get_all() {
        $this->module = get_user_module_security($this->module_key);
        $tax_groups = self::get_all();
        if (!empty($tax_groups)) {
            return $tax_groups;
        } else {
    ...
    public function api_get_list()
    {
        ...
        if (!empty($this->module->read)) {
            $user = get_loged_user();
            return TaxGroup::select('id', 'name as label')
                ->get();
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    ...
    public function api_store(Request $request) {
        ...
        if(!empty($this->module->create)) {
            $tax_group = self::create($request->all());

            if(empty($tax_group['errors'])) {
                return $tax_group;
            } else {
                return $tax_group;
            }
    ...
    public function api_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $tax_group = self::update($request->all(), $id);
