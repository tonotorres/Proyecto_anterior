...
class RegionsController extends Controller
{
     ...
     * @return Region[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Http\JsonResponse
     */
    ...
    public function api_get_list($country_id)
    {
        $this->module = get_user_module_security($this->module_key);
    ...
    public function api_store(Request $request) {
        ...
        if(!empty($this->module->create)) {
            $region = self::create($request->all());

            if(empty($region['errors'])) {
                return $region;
            } else {
                return $region;
            }
    ...
    public function api_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $region = self::update($request->all(), $id);
    ...
    private function get_all() {
        ...
        if(!empty($this->module->read)) {
            $user = get_loged_user();
            return Region::all()->load('country');
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    }
