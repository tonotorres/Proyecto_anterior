...
class IvrsController extends Controller
{
     ...
     * @return Ivr[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Http\JsonResponse
     */
    ...
    public function api_get_list()
    {
        ...
        if (!empty($this->module->list)) {
            $user = get_loged_user();
            return Ivr::select('id', 'name as label')
    ...
    public function api_get(int $id) {
        ...
        $ivr = self::get($id);
        if (!empty($ivr)) {
            return $ivr->load('ivr_options', 'tag', 'ivr_options.tag');
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    }
    ...
    public function api_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $ivr = self::update($request->all(), $id);
    ...
    private function get_all() {
        ...
        if(!empty($this->module->read)) {
            $user = get_loged_user();
            return Ivr::where('company_id', $user->company_id)->get()->load('ivr_options', 'tag', 'ivr_options.tag');
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    }
    ...
    private function get($id) {
        ...
        if (!empty($this->module->read)) {
            ...
            if($user->company_id == $ivr->company_id) {
                return $ivr;
            } else {
                return null;
            }
    ...
    private function create (Array $data) {
        ...
        if ($validator->fails()) {
            ...
            //RC: Marcamos todos los ivrs que no esten 
    ...
    public function ivr_call_list(Request $request) {
        $user = get_loged_user();
        ...
        CallListIvr::dispatch($delimeter, $company_id, $start, $end, $department_id, $user_id);
