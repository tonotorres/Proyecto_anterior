...
class ProjectsController extends Controller
{
     ...
     * @return Project[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Http\JsonResponse
     */
    ...
    public function api_get_list()
    {
        ...
        if (!empty($this->module->list)) {
            $user = get_loged_user();
            return Project::select('id', 'name as label')
    ...
    public function api_store(Request $request) {
        ...
        if(!empty($this->module->create)) {
            ...
            if(empty($project['errors'])) {
                return $project->load('account', 'project_priority', 'project_status', 'project_description', 'project_comments', 'project_comments.user', 'users', 'project_stages', 'project_stages.project_stage_status');
            } else {
                return $project;
            }
        } else {
    ...
    public function api_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $project = self::update($request->all(), $id);
    ...
    public function api_partial_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $project = self::partial_update($request->all(), $id);
    ...
    private function get_all() {
        ...
        if(!empty($this->module->read)) {
            $user = get_loged_user();
            return Project::where('company_id', $user->company_id)->get()->load('account', 'project_priority', 'project_status', 'project_description', 'project_comments', 'project_comments.user', 'users', 'project_stages', 'project_stages.project_stage_status');
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    }
    ...
    private function create (Array $data) {
        ...
        if ($validator->fails()) {
            ...
            $project_description->description = $data['description'];
            ...
            $project->users()->sync($data['users_id']);
    ...
    private function update (Array $data, int $id) {
        ...
        if ($validator->fails()) {
            ...
            if($user->company_id == $project->company_id) {
                ...
                $project->project_description->description = $data['description'];
                $project->project_description->save();
