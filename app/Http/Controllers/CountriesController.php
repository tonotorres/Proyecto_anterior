...
class CountriesController extends Controller
{
     ...
     * @return Country[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Http\JsonResponse
     */
    ...
    public function api_get_list()
    {
        ...
        if (!empty($this->module->read)) {
            $user = get_loged_user();
            return Country::select('id', 'name as label')
                ->get();
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    ...
    public function api_store(Request $request) {
        ...
        if(!empty($this->module->create)) {
            $country = self::create($request->all());
    ...
    public function api_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $country = self::update($request->all(), $id);
