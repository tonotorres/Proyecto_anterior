...
class TaxesController extends Controller
{
     ...
     * @return Tax[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Http\JsonResponse
     */
    ...
    public function api_get_list()
    {
        ...
        if (!empty($this->module->read)) {
            $user = get_loged_user();
            return Tax::select('id', 'name as label')
                ->get();
        } else {
            return response()->json(['error' => 'unauthenticated'], 401);
        }
    ...
    public function api_store(Request $request) {
        ...
        if(!empty($this->module->create)) {
            $tax = self::create($request->all());

            if(empty($tax['errors'])) {
                return $tax->load('tax_group');
            } else {
                return $tax;
            }
        } else {
    ...
    public function api_update(Request $request, int $id) {
        ...
        if(!empty($this->module->update)) {
            $tax = self::update($request->all(), $id);
